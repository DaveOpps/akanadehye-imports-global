...
const SECTIONS: Section[] = [
  ...
  },
  {
    label: "Calculators",
    items: [
      { href: "/admin/cbm-calculator", label: "CBM Calculator", icon: "cube" },
      { href: "/admin/tax-calculator", label: "Tax Calculator", icon: "calculator", superAdminOnly: true },
    ],
  },
  {
    label: "Grow",
...
export default function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const navCard = (drawer: boolean) => (
    <div className={`bg-white overflow-hidden ${drawer ? "h-full" : "border border-[color:var(--border)] rounded-xl"}`}>
      <div className="px-4 py-3 bg-[color:var(--brand-cream)] border-b border-[color:var(--border)] flex items-center justify-between">
        <div>
          <div className="text-xs uppercase tracking-wider text-[color:var(--muted)] font-semibold">Admin Dashboard</div>
          {isSuperAdmin && (
        ...
        {drawer && (
          <button onClick={() => setMobileOpen(false)} aria-label="Close menu" className="h-8 w-8 -mr-1 inline-flex items-center justify-center rounded-full text-[color:var(--muted)] hover:bg-white">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" /></svg>
          </button>
        )}
      </div>

      <nav className="p-2 space-y-4 overflow-y-auto" style={drawer ? { maxHeight: "calc(100% - 108px)" } : undefined}>
        {SECTIONS.map((section) => {
          const visible = section.items.filter(canSee);
          if (visible.length === 0) return null;
          return (
            <div key={section.label}>
              <div className="px-3 pt-2 pb-1.5 text-[10px] uppercase tracking-[0.1em] text-[color:var(--muted)] font-bold">{section.label}</div>
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition ${
                        active ? "bg-[color:var(--brand-navy)] text-white shadow-sm" : "text-[color:var(--brand-navy)] hover:bg-[color:var(--brand-cream)] active:bg-[color:var(--brand-cream)]"
                      }`}
                    >
                      <Icon name={n.icon} />
                      {n.label}
      ...
      <div className="border-t border-[color:var(--border)] px-4 py-3">
        <Link href="/" onClick={() => setMobileOpen(false)} className="inline-flex items-center gap-1.5 text-xs font-semibold text-[color:var(--muted)] hover:text-[color:var(--brand-navy)] transition">
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-5 lg:px-8 py-4 lg:py-10">
      {/* Mobile menu bar */}
      <div className="lg:hidden mb-4 flex items-center gap-3">
        <button
          onClick={() => setMobileOpen(true)}
          aria-label="Open admin menu"
          className="inline-flex items-center gap-2 rounded-lg border border-[color:var(--border)] bg-white px-3 py-2 text-sm font-bold text-[color:var(--brand-navy)] shadow-sm active:scale-95 transition"
        >
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <aside className="absolute left-0 top-0 h-full w-72 max-w-[86%] bg-white shadow-2xl">
            {navCard(true)}
          </aside>
        </div>
      )}
...
function Icon({ name }: { name: string }) {
  const paths: Record<string, React.ReactNode> = {
    ...
    calculator: <><rect x="4" y="2" width="16" height="20" rx="2" stroke="currentColor" strokeWidth="2"/><path d="M8 6h8M8 10h2M11 10h2M14 10h2M8 14h2M11 14h2M14 14v4M8 18h2M11 18h2" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></>,
    cube:     <path d="M21 16V8a2 2 0 00-1-1.73l-7-4a2 2 0 00-2 0l-7 4A2 2 0 003 8v8a2 2 0 001 1.73l7 4a2 2 0 002 0l7-4A2 2 0 0021 16zM3.27 6.96L12 12.01l8.73-5.05M12 22.08V12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>,
